...
- Validate the focused writable element through Accessibility, then use one targeted synthesized Command–V as the insertion mechanism. Do not attempt a direct Accessibility write first or perform a second insertion after unverified readback, because either sequence could duplicate text.
...
2. **Included:** Best-effort materialization of all pasteboard items/types; ownership change count; final-text clipboard write; focused element resolution only after processing; selection replacement; boundary spacing; one PID-targeted synthesized Command–V insertion; delayed guarded restoration after paste consumption; `Confirmed`/`Unverified`/`Failed`; clipboard fallback messaging; cancellation rollback while still owner.
3. **Excluded:** Remembering the original app/caret, a second insertion mechanism after an attempted paste, undo/removal after completed insertion, clipboard persistence or semantic inspection.
...
5. **Frameworks/APIs:** `NSPasteboard`/`NSPasteboardItem`; ApplicationServices focused-element/selected-range/value inspection; CoreGraphics PID-targeted keyboard events.
6. **Dependencies:** Final text from Slice 8 or raw text from Slice 6; Accessibility status from Slice 3.
7. **Manual acceptance:** Native and web-rendered editable targets receive one automatic paste; empty target and beginning caret insert unchanged text; selections are replaced; switching apps/carets during processing changes the destination; adjacent word characters gain exactly one space; punctuation/whitespace/newlines gain none; confirmed insertion restores a still-owned mixed-type clipboard snapshot after paste consumption; changing the clipboard during processing prevents restoration; denied AX/no writable target leaves final text and displays manual-paste fallback; unverified insertion leaves the transcript available.
8. **Risks:** Accessibility readback differs substantially between native and web/editor controls, and contenteditable controls may normalize inserted whitespace. Return Unverified when a posted paste cannot be proven, leave the transcript available, and never attempt a second insertion. Use UTF-16 ranges for AX values and define word-forming boundaries as Unicode letters, marks, decimal digits, or connector punctuation.
